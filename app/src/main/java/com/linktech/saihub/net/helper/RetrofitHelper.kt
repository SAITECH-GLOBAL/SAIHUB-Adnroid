...
import com.linktech.saihub.manager.AppConfigManager
...
import com.linktech.saihub.net.interceptor.CommonInterceptorWrapper
import okhttp3.*
...
class RetrofitHelper() {
    ...
    companion object {
        ...
        private fun getInstance(baseUrl: String): RetrofitHelper {
    ...
    init {
        ...
        SaiHubApplication.getInstance()?.let {
            AppConfigManager.getInstance()?.getHeadIntercepet(it)
                ?.let { it1 -> builder.addInterceptor(it1) }
        }
        builder.connectTimeout(DEFAULT_TIMEOUT_SECONDS.toLong(), TimeUnit.SECONDS)
            ...
            .addInterceptor(CommonInterceptorWrapper())
