...
public class SplitEditTextView extends AppCompatEditText {
    ...
    private int mPasContentColor;
    //每个输入框是否是正方形标识
    private boolean mInputBoxSquare;
    private boolean isfocusedShowCursor;
    ...
    private void initAttrs(Context c, AttributeSet attrs) {
        ...
        mPasContentColor = array.getColor(R.styleable.SplitEditTextView_pasContentColor, Color.BLACK);
        mInputBoxSquare = array.getBoolean(R.styleable.SplitEditTextView_inputBoxSquare, true);
        isfocusedShowCursor = array.getBoolean(R.styleable.SplitEditTextView_isfocusedShowCursor, false);
...
//        setCursorVisible(false);
    ...
    private void drawContent(Canvas canvas) {
        ...
        if (mContentShowMode == CONTENT_SHOW_MODE_PASSWORD) {
            mPaintContent.setColor(mPasContentColor);
            for (int i = 0; i < password.length(); i++) {
                float startX = getDrawContentStartX(i);
                //每次重新绘制时,先将rectF重置下
                mRectConnect.setEmpty();
                //为避免在onDraw里面创建RectF对象,这里使用rectF.set()方法
                mRectConnect.set(startX - mCircleRadius / 2, cy - mCircleRadius / 2, startX + mCircleRadius / 2, cy + mCircleRadius / 2);
                canvas.drawRoundRect(mRectConnect, PixelUtils.dp2px(4f), PixelUtils.dp2px(4f), mPaintContent);
            }
        } else {
    ...
    private void drawCursor(Canvas canvas) {
        ...
        if (isfocusedShowCursor && !isFocused()) {
            return;
        }
