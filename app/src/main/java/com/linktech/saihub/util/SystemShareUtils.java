...
public class SystemShareUtils {
    ...
    public static void shareUrl(Context context, String path) {
        ...
        context.startActivity(Intent.createChooser(it, context.getResources().getString(R.string.string_share)));
    }
     ...
     * @param pkg      分享到的指定app的包名   /storage/emulated/0/Android/data/com.linkchain.lite/files/Pictures/1632381510037.jpg
    ...
    private static void shareImage(Context context, String path, List<String> pathList, String pkg, String cls) {
        if (path == null && pathList == null) {
            ToastUtils.INSTANCE.shortToast("找不到您要分享的图片文件");
            return;
        }
        checkFileUriExposure();
        try {
            if (path != null) {
                //单张图片
                if (!isFile(path)) {
                    ToastUtils.INSTANCE.longToast("图片不存在，请检查后重试");
                    return;
                }
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                    contentUri = FileProvider.getUriForFile(SaiHubApplication.Companion.getInstance(), Constants.FILEPROVIDER_AUTHORITIES, new File(path));
                } else {
                ...
                for (int i = 0; i < pathList.size(); i++) {
                    if (!isFile(pathList.get(i))) {
                        ToastUtils.INSTANCE.longToast("第" + (i + 1) + "张图片不存在，请检查后重试");
                        return;
                    }
            ...
            ToastUtils.INSTANCE.longToast("分享失败，未知错误");
        }
    }
    ...
    public static void shareImageToWeChat(Context context, String path) {
        ...
        if (!isAppInstall(context, "com.tencent.mm")) {
            ToastUtils.INSTANCE.longToast("您还没有安装微信");
            return;
        }
    ...
    public static void shareImageToQQ(Context context, String path) {
        if (!isAppInstall(context, "com.tencent.mobileqq")) {
            ToastUtils.INSTANCE.longToast("您还没有安装QQ");
            return;
        }
    ...
    public static void shareImageToQZone(Context context, String path) {
        if (!isAppInstall(context, "com.qzone")) {
            ToastUtils.INSTANCE.longToast("您还没有安装QQ空间");
            return;
        }
    ...
    public static void shareImageToWeibo(Context context, String path) {
        if (!isAppInstall(context, "com.sina.weibo")) {
            ToastUtils.INSTANCE.longToast("您还没有安装新浪微博");
            return;
        }
    ...
    public static void jumpAppByPackageName(Context context, String pkg, String cls) {
        ...
        if (pinfo != null) {
            for (int i = 0; i < pinfo.size(); i++) {
                ...
                if (pn.equals(pkg)) {
                    ...
                    if (i == pinfo.size() - 1) {
//                        ToastUtils.INSTANCE.shortToast(context.getString(R.string.not_install));
                    }
                }
            }
        }
