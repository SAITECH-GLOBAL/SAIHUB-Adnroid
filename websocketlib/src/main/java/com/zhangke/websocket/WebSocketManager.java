...
public class WebSocketManager {
 ...
 /*       Request<String> request = RequestFactory.createStringRequest();
        LogUtil.d("send_socket", "\t");
//                    LogUtil.d("send_socket", "发送消息------->\n");
        LogUtil.d("send_socket", "发送消息" + text);
//                    LogUtil.d("send_socket", "发送消息<-------\n");
        request.setRequestData(text);
...
//        messageQueue.add(RequestFactory.createPongRequest());
     ...
     * 发送数据
     */
    private void sendRequestDelayed(Request request) {
        if (destroyed) {
            LogUtil.e(TAG, "This WebSocketManager is destroyed!");
            return;
        }
        mWebSocketEngine.sendRequest(mWebSocket, request, mSocketWrapperListener, true);
    }
    ...
    private void sendRequest(Request request) {
        ...
        if (mSetting == null) {
            ...
            mWebSocketEngine.sendRequest(mWebSocket, request, mSocketWrapperListener, mSetting.isDelayed);
        }
    }
